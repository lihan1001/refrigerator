...
app.post('/fetch_recipes', async (req, res) => { //async:非同步
    ...
    try {
        ...
        const response = await axios.post(`${API_BASE_URL}/fetch_recipes`, { ingredients });
...
app.listen(port, () => {
    console.log(`Server running on http://localhost:${API_BASE_URL}`);
});
